...
import {applyFor, shoppingDetail, updateShopping} from '@/api/operator';
...
export default function UploadData(props: any) {
  ...
  useEffect(() => {
    if (params?.shop_id) {
      ...
      shoppingDetail({
        ...
        // res.license &&
        //   res.license.join().map((item: any) => {
        //     const obj = {
        //       url: item,
        //     };
        //     businessImg.push(obj);
        //   });
        res.license &&
          businessImg.push({
            url: res.license,
        ...
        changeImgList({
          shopImg,
          businessImg,
          id_fornt_img: res.id_fornt_img,
          id_back_img: res.id_back_img,
          bank_certificate: res.bank_certificate,
        });
      });
    } else {
  ...
  const handleSubmit = () => {
    ...
    const _params = {
      ...
      id_back_img: imgs.id_back_img,
      bank_certificate: imgs.bank_certificate,
      name,
      description: remarks,
      operation_id: new Date().getTime().toString(),
    };
  ...
  return (
    <View
      ...
      <ScrollView
        ...
        <View style={{margin: pt(15), marginBottom: pt(0)}}>
          <Text style={styles.title}>{t('身份证正面')}</Text>
          <TouchableOpacity
            style={styles.idCrad}
            activeOpacity={1}
            onPress={() => {
              selectPhotoTapped(true, {selectionLimit: 1}).then((res: any) => {
                changeImgList({
                  ...imgs,
            ...
            <View>
              {imgs.id_front_img ? (
                <Image
                  ...
                  source={{uri: imgs.id_front_img}}></Image>
              ) : (
                <>
                  <View row centerH>
                    <Icon
        ...
        <View style={{margin: pt(15), marginBottom: pt(0)}}>
          <Text style={styles.title}>{t('身份证反面')}</Text>
          <TouchableOpacity
            style={styles.idCrad}
            activeOpacity={1}
            onPress={() => {
            ...
            <View>
              {imgs.id_back_img ? (
                <Image
                  ...
                  source={{uri: imgs.id_back_img}}></Image>
              ) : (
                <>
                  <View row centerH>
                    <Icon
        ...
        <View style={{margin: pt(15), marginBottom: pt(0)}}>
          <Text style={styles.title}>{t('财产证明')}</Text>
          <TouchableOpacity
            style={styles.selectImg}
            activeOpacity={1}
            onPress={() => {
              selectPhotoTapped(true, {selectionLimit: 1}).then((res: any) => {
                changeImgList({
                  ...imgs,
                  bank_certificate: formatUrl(res[0].url),
                });
              });
            }}>
            <View>
              {imgs.bank_certificate ? (
                <Image
                  resizeMode="cover"
                  style={styles.selectImg}
                  source={{uri: imgs.bank_certificate}}></Image>
              ) : (
                <>
                  <View row centerH>
                    <Icon
