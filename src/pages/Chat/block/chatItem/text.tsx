...
export default function ChatText({
  ...
  setQuoteContent: (row: any, content: string) => void;
  ...
  const onPress = (id: string, key: string) => {
    ...
    switch (key) {
      case 'forward':
        ...
        break;
      case 'quote':
        setQuoteContent(row, getMsgContent(row));
        break;
      case 'select':
      ...
      <View
        ...
        <Hint
          ...
          <Text
            ...
            {/* {content.at_info?.length
              ? _.map(content.at_info, (item, index) => {
                  return (
                    <Text key={index} style={{...textDiffStyle}}>
                      {content.text.replace(
                        `@${item.user_id}`,
                        `@${item.group_nick_name}`,
                      )}
                    </Text>
                  );
                })
                    ...
                    <Text key={index} style={textDiffStyle}>
                      <Text
                        style={{
                          fontWeight: 'bold',
                          ...atTextDiffStyle,
                        }}>{`@${item.group_nick_name}`}</Text>
                      {` ${content.text.replace(`@${item.user_id}`, '')}`}
                    </Text>
                  );
                })
              : content.text}
